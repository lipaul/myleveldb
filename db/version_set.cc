...
static double MaxBytesForLevel(const Options *options, int level)
{
    ...
    */
    double result = options->T * 1048576.0;
    while (level > 0)
    {
        result *= options->T;
        level--;
...
static uint64_t MaxFileSizeForLevel(const Options *options, int level)
{
    ...
    {
        return MaxBytesForLevel(options, level) / options->K;
    }
    else
    {
        return MaxBytesForLevel(options, level) / options->Z;
    }
}
...
void VersionSet::Finalize(Version *v)
{
    ...
    for (int level = 0; level < config::kNumLevels - 1; level++)
    {
        ...
        */
        ...
        score = v->files_[level].size() / v->vset_->options_->K;
...
Compaction *VersionSet::PickCompaction()
{
    ...
    */
    ...
    //SetupOtherInputs(c);
    int size = current_->files_[current_->compaction_level_ + 1].size();
    c->inputs_[1].push_back(current_->files_[current_->compaction_level_ + 1][size - 1]);
